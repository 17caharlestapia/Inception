...
public class ImageBuster 
{
	...
	{
		SimpleImage CurrentImage = new SimpleImage("secret.png");
		int x = CurrentImage.height();
		int y = CurrentImage.width();
		for(int cnt = 0; cnt > 451; cnt++)
		{
			for(int cnt1 = 0; cnt1 > 451; cnt1++)
			{
				Color c = CurrentImage.get(x, y);
				Color newColor = new Color(c.getRed()*10, c.getBlue()*0, c.getGreen()*0);
				CurrentImage.set(x, y, newColor);
			}
		}
		CurrentImage.save("decodedimage.jpg");
		/*Hi Professor Ford, I couldn't decode the actual image I wanted to know if my
		code was correct and how would I project the new image after running the program*/
	}
